...
class MultimodalRetriever:
    def __init__(self):
        ...
        try:
            ...
            self.blip_index = faiss.read_index(str(DATA_DIR / "blip_faiss_index.bin"))
        except RuntimeError:
    ...
    def search_blip(self, query: str, k: int = 20):
        """System B: BLIP + FAISS Semantic Retrieval"""
        self._init_blip()
        inputs = self.blip_processor(text=[query], return_tensors="pt", padding=True).to(device)
        with torch.no_grad():
            text_features = self.blip_model.get_text_features(**inputs)
            text_features = text_features / text_features.norm(p=2, dim=-1, keepdim=True)
            text_features = text_features.cpu().numpy().astype('float32')

        scores, indices = self.blip_index.search(text_features, k)
        return [{"image_id": self.idx_to_image[idx], "score": float(score)} 
                for score, idx in zip(scores[0], indices[0])]
